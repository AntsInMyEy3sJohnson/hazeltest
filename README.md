# Hazeltest
Welcome, fellow Hazelcast warrior! If you're looking at this code repository, it may be so because you've been facing the challenge of testing your Hazelcast clusters, any maybe you've been wondering if there are tools out there making that process a bit easier.
...
```bash
$ helm upgrade --install hazelcastimdg ./hazelcastimdg --namespace=hazelcastimdg
```
...
In the Hazeltest pod, you should see some logging statements informing about the duration of a bunch of `getMap()` calls the two runners enabled by default have made.
...
### Configuration
The default configuration resides right with the source code, and you can find it [here](./client/config/defaultConfig.yaml). It contains all properties currently available for configuring the two aforementioned resources along with comments shortly describing what each property does and what it can be used to.
